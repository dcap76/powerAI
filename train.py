...
def train_cnn_rnn():
	...
	x_, y_, vocabulary, vocabulary_inv, df, labels = parser.load_data(input_file)

	training_config = "./training_config.json"
	params = json.loads(open(training_config).read())

	word_embeddings = parser.load_embeddings(vocabulary)
	#json格式的 word：向量
	...
	x, x_test, y, y_test = train_test_split(x_, y_, test_size=0.1, stratify=y_)

	# Split the train set into train set and dev set
	x_train, x_dev, y_train, y_dev = train_test_split(x, y, test_size=0.1, stratify=y)

	vocabulary_switch = {y: x for x, y in vocabulary.iteritems()}
	vocabulary_train = {}
	for i in x_train:
		for j in i:
			vocabulary_train.update({vocabulary_switch[j]: j})
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			train_batches = parser.batch_iter(list(zip(x_train, y_train)), params['batch_size'], params['num_epochs'])
			...
			for train_batch in train_batches:
				...
				if current_step % params['evaluate_every'] == 0:
					dev_batches = parser.batch_iter(list(zip(x_dev, y_dev)), params['batch_size'], 1)
			...
			test_batches = parser.batch_iter(list(zip(x_test, y_test)), params['batch_size'], 1, shuffle=False)
	...
	with codecs.open(trained_dir + 'words_index.json', 'w', encoding="utf-8") as outfile:
		json.dump(vocabulary_train, outfile, indent=4, ensure_ascii=False)
	with open(trained_dir + 'embeddings.pickle', 'wb') as outfile:
		pickle.dump(embedding_mat, outfile, pickle.HIGHEST_PROTOCOL)
	with codecs.open(trained_dir + 'labels.json', 'w', encoding="utf-8") as outfile:
...
if __name__ == '__main__':
	# word2vec.word2Vec("./data/corpusSegDone.zip")
	word2vec.word2Vec("./data/yuliao.txt")
	train_cnn_rnn()
